...
def test_claude_md_seams_are_filled_after_rename() -> None:
    ...
    assert (ROOT / "CLAUDE.md").is_symlink(), "CLAUDE.md must be a symlink to AGENTS.md"
    assert (ROOT / "GEMINI.md").is_symlink(), "GEMINI.md must be a symlink to AGENTS.md"
    ...
    if _package_name() == "mytemplate":
        ...
        assert not remaining, (
            f"AGENTS.md still has unfilled template seams: {remaining} — "
